...
bool ImGui::BeginComboEx(const char* label, const char* preview_value, float width, ImGuiComboFlags flags)
{
    // Always consume the SetNextWindowSizeConstraint() call in our early return paths
    ImGuiContext& g = *GImGui;
    bool has_window_size_constraint = (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint) != 0;
    ...
    //}
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    RenderArrow(window->DrawList, ImVec2(value_x2 - 12.f - style.FramePadding.x, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.0f);
    RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(width, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
    ...
    // Horizontally align ourselves with the framed text
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(style.FramePadding.x, style.WindowPadding.y));
    SetNextWindowContentWidth(width);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if(selected)
    RenderCheckMark(ImVec2(bb_inner.Min.x + style.FramePadding.x , bb_inner.Min.y + bb_inner.GetHeight()/2.f - 4.f), GetColorU32(ImGuiCol_Text), 8.f);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (hovered && g.HoveredIdNotActiveTimer > 0.50f && bb.GetWidth() < tab->WidthContents)
    {
        ...
        TabItemBackground(display_draw_list, bb, header_flags, GetColorU32(ImGuiCol_TitleBgActive));
    }
#endif
...
void  ImGui::DragFloatNEx(const char* labels[], float* v, int components, float v_speed, float v_min, float v_max,
